...
def run(args: Any) -> int:
    ...
    if cfg.get("agent", {}).get("enabled", False):
        generated_file = cfg["agent"].get("generated_file")
        if not generated_file:
            generated_file = f"{str(cfg['backend']['generated_package']).rstrip('/')}/copilot_router.py"
        _require_file(errors, project_root / str(generated_file), "COPILOT_GENERATED_FILE_MISSING")
        mount_file = project_root / str(cfg["agent"].get("mount_file") or cfg["backend"]["mount_file"])
        _require_file(errors, mount_file, "COPILOT_MOUNT_FILE_MISSING")
        if mount_file.exists() and "filterx_copilot_router" not in mount_file.read_text(encoding="utf-8"):
