...
- The `agent` extra for natural-language Filter Copilot support
...
Set `agent.enabled: true` in `filterx.yaml`. The orchestrated `filterx install` command then installs the copilot router after the FastAPI backend. Groq and Gemini use environment-variable API keys; `openai-compatible` supports hosted providers and keyless local servers such as Ollama. See [../../AGENT_LAYER_GUIDE.md](../../AGENT_LAYER_GUIDE.md) for complete examples and tests.
